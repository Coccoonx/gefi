...
public class TransactionServiceImpl implements TransactionService {
    ...
    public Transaction create(Transaction transaction) {
        ...
        transaction.setDateCreation(date);
        transaction.setDateDerniereModification(date);
    ...
    public Transaction update(Transaction transaction) {
        logger.info("Incoming object {}", transaction);
        Transaction exist = transactionRepository.findOne(
                transaction.getId());
        if (exist == null) {
            throw new IllegalArgumentException("transaction doesn't exist");
        }

        transaction.setDateDerniereModification(new Date().getTime());
        Transaction saved = transactionRepository.save(transaction);
            ...
            for (Long j : jours) {
                ...
                for (Transaction tmp : transactions) {
                    if (tmp.getDateTransaction() == j) {
                        t.setMontantTransaction(t.getMontantTransaction()
                                + tmp.getMontantTransaction());
                    }
                }
                tuples.add(t);
